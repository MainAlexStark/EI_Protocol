...
def get_selected_table(self):
    ...
    try:
        ...
        current_tab_index = self.tab_widget_excel.currentIndex()
        current_tab = self.tab_widget_excel.widget(current_tab_index)
...
def verificationer_changed(self):
    ...
    self.text_num_protocol_excel.setPlainText(verificationers[self.verificationer_combo_excel.currentText()])
...
def create_protocol(self):
    ...
    selected_table = get_selected_table(self)

    # Получаем standarts
    standarts = selected_table.item(selected_table.currentRow(), 1).text()
    standarts_briefly = selected_table.item(selected_table.currentRow(), 4).text()
    ...
    with open(file_name, 'r+') as file:
        ...
        for text_widget_name in self.var_boxes_excel.text_boxes.keys():
            
            text = str(self.var_boxes_excel.text_boxes[text_widget_name].toPlainText()).strip()
            
            PlaceHolderText = self.var_boxes_excel.text_boxes[text_widget_name].placeholderText
        ...
        for combo_widget_name in self.var_boxes_excel.combo_boxes.keys():
            
            text = str(self.var_boxes_excel.combo_boxes[combo_widget_name].currentText()).strip()
            
            if not self.var_boxes_excel.combo_boxes[combo_widget_name].currentIndex() == -1:
        ...
        for button_widget_name in self.buttons_excel.CheckableButtons.keys():
        ...
        args['inspection_date_excel'] = data['inspection_date_excel'] = str(self.inspection_date_excel.selectedDate().toString("dd.MM.yyyy")).strip()
        args['standarts_excel'] = data['standarts_excel'] = standarts
        args['standarts_briefly_excel'] = standarts_briefly
    ...
    if not var_flag:
        if args['create_excel'].isChecked():
            ...
            # Создаем протокол
            result = EXCEL.make_new_protocol(args)
...
def use_data(self):
    ...
    for name in data.keys():
        ...
        try:
            ...
            for widget_name in self.var_boxes_excel.text_boxes.keys():
                # Если названия сходятся
                if name == widget_name:
                    # Устанавливаем значение из storage.json в текст виджета
                    self.var_boxes_excel.text_boxes[widget_name].setPlainText(data[name])
        except:
            logger.error('Ошибка при установлении значений в self.var_boxes.text_boxes')

        try:
            # Перебираем все key комбо виджетов
            for widget_name in self.var_boxes_excel.combo_boxes.keys():
                # Если названия сходятся
                if name == widget_name:
                    # Устанавливаем значение из storage.json в текст виджета
                    self.var_boxes_excel.combo_boxes[widget_name].setCurrentText(data[name])
        except:
            logger.error('Ошибка при установлении значений в self.var_boxes.combo_boxes')

        try:
            # Перебираем все key комбо виджетов
            for widget_name in self.buttons_excel.CheckableButtons.keys():
                # Если названия сходятся
                if name == widget_name:
                    # Устанавливаем значение из storage.json в текст виджета
                    self.buttons_excel.CheckableButtons[widget_name].setChecked(data[name])
        except:
    ...
    if use_data['inspection_date']:
        ...
        if inspection_date_str:
            ...
            if len(inspection_date_parts) == 3:
                ...
                self.inspection_date_excel.setSelectedDate(QDate(year, month, day))

    if use_data['standarts']:
        # Перебор всех вкладок
        for index in range(self.tab_widget.count()):

            current_tab = self.tab_widget_excel.widget(index)  # Получить виджет текущей вкладки
            ...
            for row in range(table.rowCount()):
                ...
                if item.text() == data['standarts']:
                    self.tab_widget_excel.setCurrentIndex(index)  # Установить текущую вкладку
...
def clean(self):
    ...
    for text_widget in self.var_boxes_excel.text_boxes.values():
        text_widget.clear()
    for combo_widget in self.var_boxes_excel.combo_boxes.values():
        combo_widget.setCurrentIndex(-1)
    for button_widget in self.buttons_excel.CheckableButtons.values():
